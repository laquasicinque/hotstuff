...
var normalizeHotKey = function (x) { return (KEY_ALIAS_MAP[x] ? KEY_ALIAS_MAP[x] : x); };
...
var parseKeyCode = function (str) {
    return str.toLowerCase().split(/[+-]/);
};
// helper function to make sure that there is an array for us to push our handlers to
var addKeyListener = function (T, key) {
    if (T._keyListeners[key] === undefined)
        T._keyListeners[key] = [];
    return T._keyListeners[key];
};
var getKeyObject = function (keyObj, key) {
    keyObj = keyObj._keys;
...
function createFakeEvent(event, el) {
    // @ts-ignore
    var fakeEvent = document.createEventObject
        // @ts-ignore
...
var DEFINE = function (o) { return function (p, v) { return Object.defineProperty(o, p, v); }; };
// Begin HotStuff Class
var HotStuff = function (el) {
...
Object.assign(HotStuff.prototype, {
    ...
    init: function (el) {
        ...
        if (el === null || el === undefined)
            if (window)
                ...
                throw new Error("No element found to attach HotStuff to and could not find window object.");
        ...
        (function (arr, func) {
            arr.forEach(function (x) { return window.addEventListener(x, func); });
        })(['blur', 'focus'], function () {
    ...
    addListener: function (keysArr, keydownHandler, keyupHandler) {
        var _this = this;
        ...
        if (keydownHandler == null)
            keydownHandler = function (_) { return _; };
        if (keyupHandler == null)
            keyupHandler = function (_) { return _; };
        var sharedStore = {};
        ...
        if (keysArr.length === 0) {
            ...
            this._genericKeyListeners.push({
                keyDown: function (e) {
                    _events_1.push(e);
                    return keydownHandler(e, sharedStore, _this._keysActive.slice());
                },
                keyUp: function (e) { return keyupHandler(e, sharedStore, _this._keysActive.slice()); },
                _keys: _keys,
                _events: _events_1
            });
        ...
        keysArr.forEach(function (keys) {
            ...
            var keyActiveBuffer = [];
            var _events = [];
            keydownHandler = keydownHandler.bind(_this);
            keyupHandler = keyupHandler.bind(_this);
            addKeyListener(_this, keys.join("+")).push({
                keyDown: function (e) {
                    ...
                    _events.push(e);
                    keyActiveBuffer = _this._keysActive.slice();
                    return keydownHandler(e, sharedStore, _this._keysActive.slice());
                },
                keyUp: function (e) { return keyupHandler(e, sharedStore, keyActiveBuffer); },
    ...
    handleKeyDown: function (e) {
        ...
        if (this._keyListeners[keyStr] != null)
            this._keyListeners[keyStr].forEach(function (listenerGroup) {
                if (listenerGroup.keyDown(e) !== false) {
                    // if keyDown returns false, don't add keyUp
                    if (!_this._unresolvedKeyUps.includes(listenerGroup)) {
                        _this._unresolvedKeyUps.push(listenerGroup);
                    }
                }
            });
    ...
    handleKeyUp: function (e) {
        ...
        this._unresolvedKeyUps.forEach(function (listenerGroup) { return listenerGroup.keyUp(e); });
